...
# endif
        ...
        protected virtual void GetUserCameraSettings(out float sentitivity, out bool flipX, out bool flipY)
        {
            sentitivity = 1;
            flipX = false;
            flipY = false;
        ...
        protected Vector3 CurrentTargetPosition => CurrentTarget.CameraCenter + CurrentTarget.CameraRotation * CurrentTarget.originOffset;
            ...
            if (Owner)
            {
                ...
                if (!ReferenceEquals(CurrentTarget, target))
                {
                    ...
                    if (CurrentTarget)
                    {
                        ...
                        {
                            rotation.yaw = CurrentTarget.CameraRotation.eulerAngles.y;
                ...
                if (updateInput && overrideState != OverrideState.In && CurrentTarget && CurrentTarget.allowCameraRotation && CurrentTarget.TryGetOwner(out Player owner))
                {
                    ...
                    {
                        Vector2 cameraInput = controller.Default.CameraInput.GetValue();
                        inputForce += cameraInput.magnitude;
                        if (inputForce > 1.5f) inputForce = 1.5f;
                        GetUserCameraSettings(out float sensitivity, out bool flipX, out bool flipY);
                        cameraInput *= sensitivity * deltaTime;
                        if (flipX) cameraInput.x = -cameraInput.x;
                        if (flipY) cameraInput.y = -cameraInput.y;
        ...
        protected override void UpdateTransform()
        {
            if (Application.IsPlaying(gameObject) && CurrentTarget && updatePosition)
            {
                ...
                {
                    Rotator3 targetRot = CurrentTarget.CameraRotation;
