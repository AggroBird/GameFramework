...
namespace AggroBird.GameFramework
{
    public abstract class Player : ScriptableObject
    {
        ...
        public bool TryGetCamera<T>(out T camera) where T : PlayerCamera
        {
            if (cameraStack.Count > 0 && cameraStack[^1] is T casted)
            {
                camera = casted;
                return true;
            }
        ...
        public virtual void UpdateInput(int index)
        {
            if (Controller)
            {
                ...
                if (UI)
                {
                    ...
                    UI.UpdateInput(Controller, InputEnabled);
                }
            }
        }
        ...
        protected internal virtual void OnInputModeChanged(InputMode inputMode)
        {
