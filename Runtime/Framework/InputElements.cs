...
namespace AggroBird.GameFramework
{
    public static class InputSystemUtility
    {
        ...
        public static Direction8 Direction8FromVector(Vector2 vector, float sensitivity = 0.5f)
        {
            if (vector.sqrMagnitude > sensitivity)
            {
                return Direction8FromAngle(Mathfx.AngleFromVectorDeg(vector));
            }
        ...
        {
            return (Direction8)((((int)((Mathfx.ModAbs(angle, 360f) + 22.5f) % 360f) / 45)) & 7) + 1;
        }
    ...
    [Serializable]
    public struct IntegralSwitch
    {
        public static IntegralSwitch Default => new(false);

        public IntegralSwitch(bool repeat, float repeatDelay = 0.3f, float repeatInterval = 0.1f)
        {
            this.repeat = repeat;
            this.repeatDelay = repeatDelay;
            this.repeatInterval = repeatInterval;
        ...
        public static implicit operator int(IntegralSwitch directionSwitch)
        {
            return directionSwitch.State;
        }
    }
    ...
    public sealed class KeyboardKey : InputButton
    {
        ...
        {
            platformProfile.FormatGlyph(key, output);
        }
    }
    ...
    public sealed class MouseButton : InputButton
    {
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            platformProfile.FormatGlyph(button, output);
        }
    }
    ...
    {
        public GamepadStickDirectionButton(GamepadStick stick, Direction direction, float sensitivity = 0.5f, bool glyphAsArrow = false)
        {
            this.stick = stick;
            this.direction = direction;
            this.sensitivity = sensitivity;
            this.glyphAsArrow = glyphAsArrow;
        }
        ...
        public bool glyphAsArrow = false;
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "Gamepad stick eight direction button (true/false)")]
    public sealed class GamepadStickEightDirectionButton : InputButton
    {
        public GamepadStickEightDirectionButton(GamepadStick stick, Direction8 direction, float sensitivity = 0.5f, bool glyphAsArrow = true)
        {
            this.stick = stick;
            this.direction = direction;
            this.sensitivity = sensitivity;
        ...
        [Clamped(0, 1)]
        public float sensitivity = 0.5f;
        public bool glyphAsArrow = true;

        public override bool GetValue(int index = 0)
        {
            if (direction != Direction8.None && TryGetGamepad(index, out Gamepad gamepad))
            {
                return InputSystemUtility.Direction8FromVector(gamepad.GetStickControl(stick).ReadValue(), sensitivity) == direction;
            }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "Gamepad platform confirm")]
    public sealed class GamepadPlatformConfirm : InputButton
    {
        public override bool GetValue(int index = 0)
        {
            if (TryGetGamepad(index, out Gamepad gamepad) && AppInstance.TryGetInstance(out AppInstance instance) && instance.PlatformProfile)
            {
                return gamepad[instance.PlatformProfile.GamepadPlatformConfirm].isPressed;
            }
            return false;
        }
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            if (AppInstance.TryGetInstance(out AppInstance instance) && instance.PlatformProfile)
            {
                platformProfile.FormatGlyph(instance.PlatformProfile.GamepadPlatformConfirm, output);
            }
            else
            {
                platformProfile.FormatGlyph(GamepadButtonCode.South, output);
            }
        }
    }

    [Serializable]
    [PolymorphicClassType(Tooltip = "Gamepad platform cancel")]
    public sealed class GamepadPlatformCancel : InputButton
    {
        public override bool GetValue(int index = 0)
        {
            if (TryGetGamepad(index, out Gamepad gamepad) && AppInstance.TryGetInstance(out AppInstance instance) && instance.PlatformProfile)
            {
                return gamepad[instance.PlatformProfile.GamepadPlatformCancel].isPressed;
            }
            return false;
        }
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            if (AppInstance.TryGetInstance(out AppInstance instance) && instance.PlatformProfile)
            {
                platformProfile.FormatGlyph(instance.PlatformProfile.GamepadPlatformCancel, output);
            }
            else
            {
                platformProfile.FormatGlyph(GamepadButtonCode.East, output);
            }
        }
    }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "2-directional keyboard user interface input")]
    public sealed class KeyboardInputDirection : InputDirection
    {
        ...
        public override Direction GetValue(int index = 0)
        {
            ...
            {
                int y = 0;
                if (up != KeyCode.None && keyboard[up].isPressed) y++;
                if (down != KeyCode.None && keyboard[down].isPressed) y--;
                int x = 0;
                if (right != KeyCode.None && keyboard[right].isPressed) x++;
                if (left != KeyCode.None && keyboard[left].isPressed) x--;
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            platformProfile.FormatGlyph(up, output);
            output.Append('/');
            platformProfile.FormatGlyph(right, output);
            output.Append('/');
            platformProfile.FormatGlyph(down, output);
            output.Append('/');
            platformProfile.FormatGlyph(left, output);
        }
    }

    [Serializable]
    [PolymorphicClassType(Tooltip = "2-directional gamepad user interface input")]
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "2-directional gamepad stick user interface input")]
    public sealed class GamepadStickInputDirection : InputDirection
    {
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            platformProfile.FormatGlyph(stick, output);
        }
    }

    // UI directions (1d integral)
    [Serializable]
    public abstract class IntegralDirection : InputElement
    {
        public abstract int GetValue(int index = 0);
    }

    [Serializable]
    [PolymorphicClassType(Tooltip = "1-directional keyboard user interface input")]
    public sealed class KeyboardIntegralDirection : IntegralDirection
    {
        public KeyboardIntegralDirection()
        {
            negative = KeyCode.LeftArrow;
            positive = KeyCode.RightArrow;
        }
        public KeyboardIntegralDirection(KeyCode negative, KeyCode positive)
        {
        ...
        public override int GetValue(int index = 0)
        {
            ...
            {
                int x = 0;
                if (positive != KeyCode.None && keyboard[positive].isPressed) x++;
                if (negative != KeyCode.None && keyboard[negative].isPressed) x--;
                return x;
            }
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            platformProfile.FormatGlyph(negative, output);
            output.Append('/');
            platformProfile.FormatGlyph(positive, output);
        }
    }

    [Serializable]
    [PolymorphicClassType(Tooltip = "1-directional gamepad user interface input")]
    public sealed class GamepadIntegralDirection : IntegralDirection
    {
        public GamepadIntegralDirection()
        {
            negative = GamepadButtonCode.DpadLeft;
            positive = GamepadButtonCode.DpadRight;
        }
        public GamepadIntegralDirection(GamepadButtonCode negative, GamepadButtonCode positive)
        {
            this.negative = negative;
            this.positive = positive;
        }
        ...
        public override int GetValue(int index = 0)
        {
            ...
            {
                int x = 0;
                if (gamepad[positive].isPressed) x++;
                if (gamepad[negative].isPressed) x--;
                return x;
            }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "1-directional gamepad stick user interface input")]
    public sealed class GamepadStickIntegralDirection : IntegralDirection
    {
        public GamepadStickIntegralDirection(GamepadStick stick, Axis axis, float sensitivity = 0.5f)
        {
            this.stick = stick;
            this.axis = axis;
            this.sensitivity = sensitivity;
        ...
        public override int GetValue(int index = 0)
        {
            if (TryGetGamepad(index, out Gamepad gamepad))
            {
                Vector2 vec = gamepad.GetStickControl(stick).ReadValue();
                float value = axis == Axis.Horizontal ? vec.x : vec.y;
                return value > sensitivity ? 1 : value < -sensitivity ? -1 : 0;
            }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "Single button axis (range 0 to 1)")]
    public sealed class AxisButton : LinearAxis
    {
        public AxisButton(InputButton button)
        {
            this.button = button;
        }
        ...
        {
            return button.GetValue(index) ? 1 : 0;
        }

        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            button.ToString(platformProfile, output);
        }
    }

    [Serializable]
    [PolymorphicClassType(Tooltip = "1-dimentional axis between two buttons (range -1 to 1)")]
    public sealed class ButtonLinearAxis : LinearAxis
    {
        public ButtonLinearAxis(InputButton negative, InputButton positive)
        {
            this.negative = negative;
            this.positive = positive;
        }
        ...
        {
            float result = 0;
            if (positive.GetValue(index)) result++;
            if (negative.GetValue(index)) result--;
            return result;
        }

        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            negative.ToString(platformProfile, output);
            output.Append('/');
            positive.ToString(platformProfile, output);
        }
    }
    ...
    public sealed class GamepadStickLinearAxis : LinearAxis
    {
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            platformProfile.FormatGlyph(stick, axis, output);
        }
    }

    [Serializable]
    [PolymorphicClassType(Tooltip = "1-dimentional mouse delta input")]
    public sealed class MouseDeltaLinearAxis : LinearAxis
    {
        public MouseDeltaLinearAxis(Axis axis)
        {
            this.axis = axis;
        }
        ...
        public override float GetValue(int index = 0)
        {
            ...
            {
                value = axis == Axis.Vertical ? mouse.delta.ReadValue().y : mouse.delta.ReadValue().x;
            }
            else
            {
                value = 0;
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            platformProfile.FormatGlyphMouse(output, axis);
        }
    }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "2-dimentional button vector input")]
    public sealed class ButtonVectorAxis : VectorAxis
    {
        public ButtonVectorAxis()
        {
            up = new KeyboardKey(KeyCode.UpArrow);
            right = new KeyboardKey(KeyCode.RightArrow);
            down = new KeyboardKey(KeyCode.DownArrow);
            left = new KeyboardKey(KeyCode.LeftArrow);
        }
        public ButtonVectorAxis(InputButton up, InputButton right, InputButton down, InputButton left)
        {
            this.up = up;
            this.right = right;
            this.down = down;
        ...
        [SerializeReference, PolymorphicField]
        public InputButton up;
        [SerializeReference, PolymorphicField]
        public InputButton right;
        [SerializeReference, PolymorphicField]
        public InputButton down;
        [SerializeReference, PolymorphicField]
        ...
        public override Vector2 GetValue(int index = 0)
        {
            ...
            if (TryGetKeyboard(index, out Keyboard keyboard))
            {
                if (up.GetValue(index)) value.y++;
                if (right.GetValue(index)) value.x++;
                if (down.GetValue(index)) value.y--;
                if (left.GetValue(index)) value.x--;
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            up.ToString(platformProfile, output);
            output.Append('/');
            right.ToString(platformProfile, output);
            output.Append('/');
            down.ToString(platformProfile, output);
            output.Append('/');
            left.ToString(platformProfile, output);
        }
    }
    ...
    public sealed class MouseDeltaVectorAxis : VectorAxis
    {
        ...
        public override void ToString(PlatformProfile platformProfile, StringBuilder output)
        {
            platformProfile.FormatGlyphMouse(output);
        }
    }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "Keyboard single key axis (range 0 to 1)"), Obsolete]
    public sealed class KeyboardAxisButton : LinearAxis
    {
        ...
        public override float GetValue(int index = 0)
        {
            return key != KeyCode.None && TryGetKeyboard(index, out Keyboard keyboard) ? keyboard[key].ReadValue() : 0;
        }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "Mouse single button axis (range 0 to 1)"), Obsolete]
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "Gamepad single button axis (range 0 to 1)"), Obsolete]
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "1-dimentional keyboard axis between two keys (range -1 to 1)"), Obsolete]
    public sealed class KeyboardKeyLinearAxis : LinearAxis
    {
        public KeyboardKeyLinearAxis(KeyCode negative, KeyCode positive)
        {
            this.negative = negative;
            this.positive = positive;
        }
        ...
        public override float GetValue(int index = 0)
        {
            return TryGetKeyboard(index, out Keyboard keyboard) ?
                ((positive != KeyCode.None ? keyboard[positive].ReadValue() : 0) - (negative != KeyCode.None ? keyboard[negative].ReadValue() : 0)) :
                0;
        }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "1-dimentional gamepad axis between two buttons (range -1 to 1)"), Obsolete]
    public sealed class GamepadButtonLinearAxis : LinearAxis
    {
        public GamepadButtonLinearAxis(GamepadButtonCode negative, GamepadButtonCode positive)
        {
            this.negative = negative;
            this.positive = positive;
        }
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "2-dimentional keyboard vector input"), Obsolete]
    public sealed class KeyboardVectorAxis : VectorAxis
    {
        ...
        public override Vector2 GetValue(int index = 0)
        {
            ...
            if (TryGetKeyboard(index, out Keyboard keyboard))
            {
                if (up != KeyCode.None && keyboard[up].isPressed) value.y++;
                if (right != KeyCode.None && keyboard[right].isPressed) value.x++;
                if (down != KeyCode.None && keyboard[down].isPressed) value.y--;
                if (left != KeyCode.None && keyboard[left].isPressed) value.x--;
    ...
    [Serializable]
    [PolymorphicClassType(Tooltip = "2-dimentional gamepad D-Pad vector input"), Obsolete]
